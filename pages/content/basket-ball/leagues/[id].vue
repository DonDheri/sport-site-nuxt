...
<template>
    ...
    <div class="border-2 mx-48 grid grid-cols-5 gap-5 px-10 pb-5 rounded-xl my-6 p-5">
        ...
        <NuxtLink :to="`/content/${currentSport?.url}/leagues/teams/${team.id}`" v-for="team in teams" class="text-2xl flex flex-col border-2 rounded-xl place-items-center"><img class="w-[150px]" :src="team.logo" alt="team logo"> {{ team.name }}</NuxtLink>
    </div>
