...
<template>
    <div class="border-2 mx-48 grid grid-cols-12 grid-rows-2 gap-5 overflow-x-scroll px-10 p-5 rounded-xl my-6">
        ...
        <NuxtLink v-for="(sport) in sports" class="flex items-center justify-center p-2 col-span-1 bg-gradient-to-r from-blue-500 to-blue-600 rounded bg-zinc-600 text-l font-medium text-white" :key="sport.id" :to="`/content/${sport.url}`">{{ sport.title }}</NuxtLink>
    </div>
</template>